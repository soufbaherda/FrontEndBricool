...
export default function ProfilSet() {
  ...
  const save = () => {
    ...
    fetch("http://localhost:8080/register", requestOptions).then(update);
  };
  const changemdp = () => {
    if (mdp !== compte.mdp) {
      setMdperror(true);
      ...
      {securite === "securite" ? (
        <>
          <div className="col-md-6">
            <label className="labels">Mot de passe actuel</label>
            <input
              type="password"
          ...
          <div className="row mt-2">
            ...
            {mdpsucc === true ? (
              <p className="success">mot de passe a été changé!</p>
            ) : (
              <></>
            )}
          </div>
        ...
        <div className="row">
          <div className="col-md-3 border-right">
            <div className="d-flex flex-column align-items-center text-center p-3 py-5">
              <Avatar
                {...stringAvatar(compte.nom + " " + compte.prenom)}
                sx={{ width: 56, height: 56 }}
              />{" "}
              <span className="font-weight-bold">{compte.username}</span>
              <span className="text-black-50">{compte.email}</span>
              <span></span>
            </div>
          </div>
